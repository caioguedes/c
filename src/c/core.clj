...
(defn -main [& args]
  (let [bootstrap (load-bootstrap)
    ...
    (println (c/yellow "Spring Cloud Config:"))
    (println (c/yellow "Url:") (-> config create-config-url replace-variables))
    ;; (replace-variables (create-config-url config))
    (println "")
    (println (c/yellow "Spring Cloud Vault:"))
    (println (c/yellow "Token:") (replace-variables (:token vault)))
    (println (c/yellow "Url:") (-> vault create-vault-url replace-variables))))
